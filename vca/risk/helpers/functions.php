...
function clearRiskMigHistroy($externalID, $typeID, $siteID) {
	$sql = "DELETE r.* FROM tdbRiskMigrationData r 
			JOIN ( SELECT r2.riskMigID FROM tdbRiskMigrationData r2 
			WHERE r2.riskMigExternalID = 1450 AND r2.riskMigType = 1 
			ORDER BY r2.riskMigID DESC LIMIT 5,100) r2 
			ON r.riskMigID = r2.riskMigID 
			WHERE r.riskMigExternalID = '1450' 
			AND r.riskMigType = '1'";
	$res = mysql_query($sql);
}

function updateRiskMigration($externalID, $typeID, $siteID) {
	$sql = "UPDATE tdbRiskMigrationData SET `riskMigSiteID` = '$siteID' WHERE riskMigExternalID = '$externalID' AND riskMigType = '$typeID'";
	$res = mysql_query($sql);
        ...
        while ($row = mysql_fetch_array($res)) {
            ...
            if($row['employeeID'] != null){
                ...
                if($type != null AND $value != null){
                	if(!array_key_exists($value, $stakeholderResult[$type])){
