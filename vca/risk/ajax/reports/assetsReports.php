...
include_once($_SERVER['DOCUMENT_ROOT'] . "/managetpc/risk/helpers/functions.php");
...
foreach ($allAssetTypes as $assetTypeIndex => $assetTypeValue){
	...
	while ($dataAssetDB = mysql_fetch_array($res)) {
	    ...
	    $sqlMig = " SELECT r.* FROM `tdbRiskMigrationData` as r 
	                ...
	                WHERE m.riskMigExternalID = " . $dataAssetDB['nodeID'] . " AND riskMigType = 1) AND riskMigType = 1";
